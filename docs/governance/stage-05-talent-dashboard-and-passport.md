...
| S05-04 | Education and qualifications | **Missing — scope unclear, deferred** | Confirmed via repo-wide search: no table, column, or UI anywhere. P1, held out of Phase B pending a founder scoping conversation alongside S05-10 (see below). |
...
| S05-10 | Job/service preference settings | **Missing — scope unclear, deferred by founder decision** | No preferences table/column anywhere. Matching already runs directly off `talent_profiles.category`/`skills` from onboarding — there's no separate preferences layer, and no existing notification-preference infrastructure to hook a new setting into. Explicitly deferred per your 2026-09-14 decision (see decision log) rather than guessed at. |
...
## Needs a founder decision (both still open)

- **S05-04** (education) and **S05-10** (job/service preferences) —
  neither has an existing table/UI/infrastructure to extend, and
  guessing the shape risks building the wrong thing. S05-10 was
  explicitly deferred per your 2026-09-14 decision; S05-04 was held
  back alongside it for the same reason. Worth one scoping conversation
  covering both before either gets built.
