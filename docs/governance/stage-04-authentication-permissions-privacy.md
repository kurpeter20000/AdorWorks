...
Status: **12 of 14 resolved**, the remaining 2 improved but deliberately
left open-ended. S04-01 through S04-05, S04-08 through S04-13 are all
built and verified (S04-04/S04-11 required no new work — already
correct designs). S04-12 (admin force-reauth) added and verified live.
S04-06/S04-14 got real new coverage (an e2e cross-org test, and a new
real-HTTP authorization integration suite for `backend/api`) but are
deliberately bounded samples, not exhaustive — flagged as ongoing work.
S04-07 (direct Postgres-role RLS testing, e.g. pgTAP) is untouched — a
categorically different, larger effort than this batch scoped.
...
| S04-06 | Server-side role/org permission tests | **Improved, still not exhaustive** | `platform/e2e/authorization.spec.ts` now has 4 real negative-path tests — added a genuinely missing one, an employer cannot edit another organisation's opportunity (verified live against the test project: 404, matching the page's `.eq("organisation_id", org.id)` scoping). `backend/api/src/authorization.integration.test.js` (new) closes the specific gap flagged in this audit: real HTTP requests against the real Express app (not mocked routing) for one representative route per gate type (`requireStaff`, `requireFinanceStaff`, `requireAdmin`), each checked for no-token/invalid-token/wrong-role — 10 tests, all passing. Deliberately a bounded sample (14 route files, dozens of endpoints), not full coverage. |
| S04-07 | Database RLS policy automated tests | **Still missing** | Unchanged by this pass — S04-06/S04-14's new tests cover the HTTP/app layer, not direct Postgres-role-level RLS testing (e.g. pgTAP). Still nothing connects as different Postgres roles to assert row-level visibility against the RLS policies themselves; a categorically different, larger effort than what this batch scoped. |
...
| S04-12 | Session expiry and revocation | **Complete (within Supabase's real limits)** | Idle-session revocation was already real (`proxy.ts:60-68`). Added the admin capability: `POST /api/people/:id/force-reauth` (`backend/api/src/routes/people.js`) sets a fresh random password, invalidating that account's refresh tokens — the actual achievable version of "sign out everywhere," since Supabase's access tokens are stateless JWTs with no instant per-user revocation (confirmed against Supabase's own docs, not assumed). Wired into the staff console (`staff/js/people.js`, a "Force re-auth" button per account, with a confirm prompt). Verified live: the underlying admin API call tested directly against the test project, succeeded, then the seeded account's password was restored to its documented value. Lint/tests clean. |
...
| S04-14 | Unauthorized-access tests across protected routes | **Improved, still not exhaustive** | Same work as S04-06 — `backend/api/src/authorization.integration.test.js` is specifically what closes the previously-noted gap ("no test hits a requireStaff-gated route over real HTTP"). Still a bounded sample, not every route. |
...
- **S04-07** — direct Postgres-role RLS testing (e.g. pgTAP). Genuinely
  large (49 tables) and a different testing approach entirely from
  everything else in this stage — worth its own dedicated pass, not
  squeezed into this batch.
- **S04-06 / S04-14** — real progress made (see the table above), but
  still only a bounded sample of the app's full route/role surface.
  Worth extending incrementally as new high-risk routes are added,
  not treated as ever fully "done."
...
compromised account. Built as a small addition to the staff console
(a "Force re-auth" button) rather than the fuller "sign out
everywhere, immediately" capability the tracker item's wording
implies, since that fuller version isn't actually possible without
infrastructure this project doesn't have (a revocation-check on every
request). **Built and verified live** — see the S04-12 row above.
